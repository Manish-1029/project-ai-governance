[🏠 Document Start](../README.md) / [Server Reports](README.md) / EMIR
...
NonReportingPartyIDType | Value "LEI", if the broker's client who has performed the trade is [marked (#client-type)](../Clients-and-Trading-Accounts/Clients.md#client-type) as "Corporate" or "Fund". Value "CLC" for all other client types. | Value "LEI".  
NonReportingPartyLEI | If "LEI" is specified in the NonReportingPartyIDType field, the value of "LEI number for EMIR reports" from the [client record (#company)](../Clients-and-Trading-Accounts/Clients.md#company) will be used. If "CLC" is specified in "NonReportingPartyIDType", the value is determined as follows (in order of priority):

  * External client ID from [trading account settings](../Clients-and-Trading-Accounts/Account-Trading-Settings.md).
  * Internal client ID, which is equal to the [client record](../Clients-and-Trading-Accounts/Clients.md) ID in MetaTrader 5.
  * Trading account ID, i.e. [account](../Clients-and-Trading-Accounts/README.md) login/number.
...
NonReportingPartyCountry | The value is determined as follows:
  ...
  * For legal entities — from the "Registration country" field of the [client record (#company)](../Clients-and-Trading-Accounts/Clients.md#company).
  * For individuals — from the "Country" field of the [client record (#address)](../Clients-and-Trading-Accounts/Clients.md#address).
...
UTI | Consists of three parts: [E02][LEI][Transaction ID]:
  ...
  * Transaction ID — [deal ticket (#deals)](../Clients-and-Trading-Accounts/Account-History.md#deals) in MetaTrader 5.
...
| Consists of three parts: [E02][LEI][Transaction ID]:
  ...
  * Transaction ID — [deal ID in the external system (#deals)](../Clients-and-Trading-Accounts/Account-History.md#deals).
...
To install the report, navigate to the Plugins section in MetaTrader 5 Administrator. After the server with the installed plugin has been selected, click "![Add](images/add_button.png)Add".
...
The following parameters are available here:
  ...
  * Mail Login List — the list of manager logins, to which [report generation errors (#errors)](EMIR.md#errors) will be emailed.
...
All missing data errors are recorded to the [trade server journal](../Managing-Trade-Server-Settings/Trade-Server-Journal.md). To request relevant logs, use the "EMIR Reporter" and "EMIR CME Manual" keywords. Error reports are additionally [sent via the internal email system (#notifications)](EMIR.md#notifications).
...
The email subject is "EMIR report automated generation status". It contains the following details:
  ...
  * If the report could not be created, the email will state: "failed to generate report (see trade server log by keyword EMIR for details)". In this case, you should request the [trade server journal](../Managing-Trade-Server-Settings/Trade-Server-Journal.md) using the "EMIR" keyword.
...
To install the reports, open the Reports section of MetaTrader 5 Administrator. After selecting a server for report installation, "![Add](images/add_button_1.png)Add".
...
NonReportingPartyIDType | Code for the external counterparty ID type | For the rows describing the external transaction part the value "LEI" is always used. For the rows describing the transaction on the MetaTrader 5 side:
  ...
  * The value "LEI" if the broker client who performed the transaction is [marked (#client-type)](../Clients-and-Trading-Accounts/Clients.md#client-type) as "Corporate" or "Fund"
...
ProductType | Financial product type | Filled in accordance with the deal [symbol type (#calculation)](../Trading-Operations/Market-Watch.md#calculation):
...
ProductClassification | Product classification in accordance with the [ISO 10692](https://www.iso.org/standard/44799.html) standard (for CFI) | Filled using the "CFI" field from the relevant [financial instrument settings (#specification)](../Trading-Operations/Market-Watch.md#specification)  
...
UTI | Unique Transaction Identifier | Consists of three parts: [E02][LEI][Transaction ID]:
  ...
  * Deal ID — for A-book it is the [deal ID In the external system (#deals)](../Clients-and-Trading-Accounts/Account-History.md#deals), for B-book it is [deal ticket (#deals)](../Clients-and-Trading-Accounts/Account-History.md#deals) in MetaTrader 5
