[🏠 Document Start](../../README.md) / [Trading Operations](../README.md) / [For Advanced Users](../For-Advanced-Users.md) / Margin Calculation: Retail Forex, CFD, Futures — Hedging

[Previous](Margin-Calculation-Retail-Forex-CFD-Futures-—-Netting.md) | [Next](Margin-Calculation-Stock-Exchange.md)

<a id="retail-forex-cfd-futures-hedging"></a>
# Retail Forex, CFD, Futures — Hedging (#retail-forex-cfd-futures-hedging)

This margin calculation model is used for Retail Forex, CFD and Futures with the [hedging position accounting system (#hedging)](../Basic-Principles.md#hedging). Unlike to the same model for netting accounts, the presence of [multiple positions/orders of one symbols (#hedged)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#hedged) is additionally taken into account in this calculation method.
...
The margin amount is calculated in several stages:
  ...
  * [Accounting multiple positions/orders of the same symbol (#hedged)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#hedged)
...
![Margin rate](images/margin_rates_1.png)
...
Their volumes are summed up and the wighted average open price is calculated for them. The resulting values are used for calculating margin by the formula corresponding to the [symbol type (#main)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#main). Margin is calculated separately for pending orders (if the margin ratio is non-zero).
...
The CFD Leverage calculation type is used for the instrument, its contract size is 5,000, and the account leverage is 1:100. The margin ratio for all types of operations is 1.
  ...
  * Calculating margin using the [CFD Leverage formula](Margin-Calculation-Basic.md) for the aggregate volume: 3 * 5 000 * 15.433333333 / 100 = 2 315.00
...
The resulting margin value is calculated as the sum of margins calculated at each step. Calculation for uncovered volume
  ...
  * The calculated volume and weighted average price are used then to calculate margin by the appropriate formula corresponding to the [symbol type](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md).
  * When considering a [margin ratio (#rate)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#rate), the larger leg ratio (buy or sell) is used.
  * The weighted average rate value is used when [converting from a margin currency to a deposit one (#conversion)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#conversion).

Calculation for covered volume Used if the "Hedged margin" value is specified in [contract specification (#specification)](../Market-Watch.md#specification). In this case margin is charged for hedged, as well as uncovered volume. If the initial margin is specified for a symbol, the hedged margin is specified as an absolute value (in monetary terms). If the initial margin is not specified (equal to 0), the contract size is specified in the "Hedged" field. The margin is calculated by the appropriate formula in accordance with the type of the financial instrument, using the specified contract size. For example, we have two positions Buy EURUSD 1 lot and Sell EURUSD 1 lot, the contract size is 100,000. If the value of 100,000 is specified in the "Hedged field", the margin for the two positions will be calculated as per 1 lot. If you specify 0, no margin is charged for the hedged (covered) volume.
...
Margin per each hedged lot of a position is charged in accordance with the value specified in the "Hedged Margin" field in the [contract specification (#specification)](../Market-Watch.md#specification):
  ...
  * The calculated volume, weighted average price and the hedged margin value are used then to calculate margin by the appropriate formula corresponding to the [symbol type](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md).
  * When considering a [margin ratio (#rate)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#rate), the average value of the buy and sell order ratios is used: (Buy rate + Sell rate)/2
  * The weighted average rate value is used when [converting from a margin currency to a deposit one (#conversion)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#conversion).
...
Calculation for pending orders
  ...
  * The weighted average rate value and the conversion rate for each pending order type is used when taking into account the [margin rate (#rate)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#rate) and the [margin to deposit currency conversion rate (#conversion)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#conversion).

Calculation specifics for hedging orders when using fixed margin When an order opposite to an existing position is placed, the margin on the hedged volume is always calculated using the "Hedge margin" value. For the non-hedged volume, the "Initial margin" value is used when placing an order, and "Maintenance margin" is applied after the appropriate position is opened. These specifics are only valid for symbols with the specified initial and maintenance margin ([Fixed Margin (#fixed)](Margin-Calculation-Basic.md#fixed) or [Futures (#futures)](Margin-Calculation-Basic.md#futures) calculation type). For example, the following parameters are used for EURUSD:
...
Long side calculation
  ...
  * The [margin rate (#rate)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#rate) for long positions is taken into account.
  * The weighted average rate value is used when [converting from a margin currency to a deposit one (#conversion)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#conversion).
...
A similar calculation is performed for each type of pending Buy orders: separately for Buy Limit, Buy Stop and Buy Stop Limit orders. All calculated values ​​are summed. Short side calculation
  ...
  * The [margin rate (#rate)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#rate) for short positions is taken into account.
  * The weighted average rate value is used when [converting from a margin currency to a deposit one (#conversion)](Margin-Calculation-Retail-Forex-CFD-Futures-—-Hedging.md#conversion).
...
Hedged margin size = 100000. Buy margin rate = 2, for Sell = 4. Calculate uncovered volume: Sell volume (3) - Buy volume (2) = 1 Calculate the weighted average Open price for the hedged volume by all positions: (1.11943 * 1+1.11953 * 1+1.11943 * 1+1.11953 * 1+1.11943 * 1)/5 = 5.59735/5= 1.11947 Calculate the weighted average Open price for the non-hedged volume by all positions: (1.11943 * 1 + 1.11943 * 1 + 1.11943 * 1)/3 = 1.11943 Calculate the margin ratio for the hedged volume: (buy ratio + sell ratio)/2 = (2 + 4)/2 = 3 The larger leg (sell) margin ratio is used for the non-hedged volume (sell): 4. Calculate the hedged volume margin using the equation: (2.00 lots * 100000 EUR * 1.11947 * 3) / 500 = 1343.36 Calculate the non-hedged volume margin using the equation: (1.00 lot * 100000 EUR * 1.11943 * 4) / 500 = 895.54 The final margin size: 1343.36 + 895.54 = 2238.90 | Example The following positions and orders exist on the account:
...
The Forex calculation type is used for the instrument, its contract size is 100000, the account leverage is 1:100 and the deposit currency is USD. The margin ratio for all types of operations is 1. Calculating the margin for the long side:
  ...
  * Calculating margin using the [Forex formula](Margin-Calculation-Basic.md) for the total volume: 3 * 100000 / 100 = 3000 EUR
  * Converting the resulting value to USD using the weighted average conversion rate: 3000 * 1.16209333 = 3486.28
  * Calculating margin for the Buy Limit order using the [Forex formula](Margin-Calculation-Basic.md): 1 * 100000 / 100 = 1000 EUR
...
Calculating the margin for the short side:
  ...
  * Calculating margin using the [Forex formula](Margin-Calculation-Basic.md) for the total volume: 2 * 100000 / 100 = 2000 EUR
  ...
  * Calculating margin using the [Forex formula](Margin-Calculation-Basic.md) for the total volume of Sell Stop orders: 2 * 100000 / 100 = 2000 EUR
